...
if __name__ == "__main__":
    ...
    coord_df = pd.read_csv(args.coordinate_path, sep='\t')
    gene_df = parse_gene_dict(args.gene_dict)
    ...
    pd.DataFrame(log_entries).to_csv(output_path / "chunking_log.txt", sep='\t', index=False)
    # also save all chunks combined into a single file
    chunked_df.to_csv(output_path / "all_chunks_combined.txt", sep='\t', index=False)
